...
This project configures HashiCorp Vault to enable the Terraform Enterprise (TFE) secrets engine for managing HCP Terraform Team tokens dynamically. It also configures JWT authentication to allow GitHub Actions (or other GitHub access) to securely authenticate to Vault and retrieve these secrets.
