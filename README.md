...
To retrieve the dynamic HCP Terraform Team token from Vault within your GitHub Actions workflow, use the official [HashiCorp Vault GitHub Action](https://github.com/hashicorp/vault-action).

Here is an example job configuration demonstrating how to securely fetch the token and export it for use by the Terraform CLI (`TF_API_TOKEN`):

```yaml
jobs:
  terraform:
    runs-on: ubuntu-latest
    # The id-token: write permission is strictly required for OIDC (JWT) authentication
    permissions:
      contents: read
      id-token: write
    ...
    steps:
      ...
      - name: Retrieve HCP Terraform Token from Vault
        uses: hashicorp/vault-action@v3
        with:
          url: $\{{ secrets.VAULT_ADDR }}
          method: jwt
          namespace: admin/tfe_demo # Replace with your fully-qualified target Vault namespace
          role: $\{{ github.event.repository.name }}
          jwtGithubAudience: vault.workload.identity
          secrets: |
            terraform/creds/$\{{ github.event.repository.name }} token | TF_API_TOKEN
```
